function bunnyKill(arr) {
    let coordinatesOfTheBombBunniesAsArr = arr.pop()
        .split(' ')
    ...
    for (let i = 0; i < coordinatesOfTheBombBunniesAsArr.length; i++) {
        ...
        let damageOfCurrExplosion = matrixOfHangars[currBombBunnyRow][currBombBunnyCol];
        ...
        for (let j = startRow; j <= endRow; j++) {
            ...
            let endCol = Math.min(currBombBunnyCol + 1, matrixOfHangars[0].length - 1);
